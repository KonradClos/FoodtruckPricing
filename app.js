...
const btnExportRecipesSummaryCsv = $("btnExportRecipesSummaryCsv"); // optional button (see note)
...
function handleCostsStdInputRaw() { /* nur tippen */ }
...
/* =========================
   Helper: Calc fields for CSV exports
   ========================= */
function computeRecipeCalcFields(recipe) {
  ...
  const costRes = calcCostResult(data, "recipe", recipe, "de");
  if (!costRes.ok) return out;

  const c = costRes.result;
  const total = Number(c.costs.totalCostPerPortion);
  if (!Number.isFinite(total)) return out;

  out.costTotal = String(total).replace(".", ",");
  ...
  if (mode === "pct") {
    ...
    if (!price.ok) return out;

    out.minGrossRounded = String(price.grossRounded).replace(".", ",");
    out.minNetImplied = String(price.netImplied).replace(".", ",");
    const dbe = price.netImplied - total;
    const dbp = price.netImplied > 0 ? dbe / price.netImplied : 0;
    out.dbEuro = String(dbe).replace(".", ",");
    out.dbPct = String(dbp).replace(".", ",");
    out.ok = true;
    return out;
  }

  // euro mode: needs TargetDBEuro
  const targetEuro = Number(recipe?.pricing?.targetDBEuro);
  if (!Number.isFinite(targetEuro) || targetEuro <= 0) return out;

  const resEuro = calcProductResult(data, "recipe", recipe, "de");
  if (!resEuro.ok) return out;

  const p = resEuro.result.pricing;
  out.minGrossRounded = String(p.grossRounded).replace(".", ",");
  out.minNetImplied = String(p.netImplied).replace(".", ",");
  out.dbEuro = String(p.dbEuro).replace(".", ",");
  out.dbPct = String(p.dbPct).replace(".", ",");
  out.ok = true;
  return out;
}
...
// DB%: tippen erlauben, normalisieren bei blur
...
// Markt/Verkauf: raw speichern beim tippen, normalisieren bei blur
...
calcMarketGross?.addEventListener("input", () => {
  ...
  if (!r) return;
  if (!r.pricing) r.pricing = {};
  r.pricing.marketGross = calcMarketGross.value;
  saveData(data);
});
...
calcSellGross?.addEventListener("input", () => {
  ...
  if (!r) return;
  if (!r.pricing) r.pricing = {};
  r.pricing.sellGross = calcSellGross.value;
  saveData(data);
});
...
btnExportIngredientsCsv?.addEventListener("click", () => {
  ...
  for (const ing of (data.catalog.ingredients || [])) {
    rows.push([
      ing.id || "",
      ing.name || "",
      "", // optional (recipe-level); column stays for Excel compatibility
      (ing.baseUnit || "kg"),
...
// Rezepte Export (detailliert, long-format, mit Kalkulation)
btnExportRecipesCsv?.addEventListener("click", () => {
  ...
  for (const r of (data.products.recipes || [])) {
    ...
    const calc = computeRecipeCalcFields(r);
...
// Rezepte Export (summary: 1 Zeile pro Rezept, mit Kalkulation)
btnExportRecipesSummaryCsv?.addEventListener("click", () => {
  const rows = [
    [
      "Recipe_ID","Recipe_Name","VatCategory","LossPct","PackagingSetId","TargetDBEuro",
      "Cost_Total_EUR","MinGrossRounded_EUR","MinNetImplied_EUR","DB_EUR","DB_Pct",
  ...
  for (const r of (data.products.recipes || [])) {
    ...
    const calc = computeRecipeCalcFields(r);
    const linesCount = Array.isArray(r.ingredients) ? r.ingredients.length : 0;
  ...
  downloadCsv("recipes_summary.csv", rows);
});

// CSV Import (Ingredients + Recipes long format)
