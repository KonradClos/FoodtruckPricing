...
/** ===== units =====
 * Ingredient baseUnit is now expected: kg | l | pc
 * Recipe lines can be: kg | g | mg | l | ml | pc
 */
const UNIT_GROUP = {
  kg: "mass",
  g: "mass",
...
export function toBaseUnit(qty, unit, baseUnit, lang = "de") {
  ...
  if (UNIT_GROUP[u] === "mass") {
    // convert to kg or g or mg as requested base
    const toGrams = (val, unit) => unit === "kg" ? val * 1000 : unit === "g" ? val : unit === "mg" ? val / 1000 : NaN;
    const fromGrams = (g, unit) => unit === "kg" ? g / 1000 : unit === "g" ? g : unit === "mg" ? g * 1000 : NaN;
...
/** Recipe cost is PER PORTION now */
export function calcRecipeIngredientCostPerPortion(data, recipe, lang = "de") {
  ...
  for (const line of recipe?.ingredients || []) {
    ...
    const baseUnit = ing.baseUnit; // kg | l | pc
...
/** €-Mode helper (still validates targetEuro if used) */
